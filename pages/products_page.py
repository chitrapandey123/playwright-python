...
class ProductsPage(BasePage):
    ...
    def assert_all_product_images_loaded(self):
        """Assert that all product images are loaded correctly without broken image placeholders"""
        items = self.inventory_items.all()
        assert len(items) > 0, "No products found on the page"
        for item in items:
            ...
            # Verify image is not a broken image placeholder
            assert "sl-404" not in src.lower(), "Product image appears to be broken (404 placeholder)"

    # -- Generated by TC Generator --
    def logout(self):
        """Click the logout link to log out from the application"""
